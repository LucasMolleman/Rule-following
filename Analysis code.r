...
### show proportions of compliance (with associated sample sizes; pie charts Fig. 1 made in PowerPoint)
means
Ns

### winsorize extremely slow responses (with payoff 0)
a$move1<-ifelse(a$move1<20000, a$move1, 20000)
# check responses to traffic light signal (after 12 seconds)
hist(a$move1)


### DESCRIPTIVES (TABLE S2 shows the key variables here) #######
stargazer(a, type="html", out="Table S2.html")
...
##### do the same for the abstract 'cross' task (Fig. 1B,D). NB: signal not after 12 seconds, but after 8.1)
...
# report=('vcsp'),
#	out="Table S3_robustness.html")


### Data shown in Figure 2 (social expectations and conditional compliance) ###

### normative expectations (normative beliefs, b_n); Fig. 2A
...
### plot descriptive expectations (descriptive beliefs b_d); Fig. 2B ###
...
### find the mode descriptive expectation b_d
Mode <- function(x) {
  ux <- unique(x)
...
### plot preferences for compliance conditional on normative expectations (n(b_n); Fig. 2C) ###
...
### plot preferences for compliance conditional on descriptive expectations (n(b_d); Fig. 2D) ###
...
### model 2 in Table S4
m1_descriptive<-lmer(comply~condition+age+male + (1|participant), data=b)
data_descriptive<-b

### combined model (model 3 in Table S4)
data_normative$treatment<-0
data_descriptive$treatment<-1

b<-rbind(data_normative, data_descriptive)
...
class(m1_descriptive) <- "lmerMod"
class(m1_combined) <- "lmerMod"
### this is Table S4 in full
stargazer(m1_normative, m1_descriptive, m1_combined, type="html", ci=TRUE, ci.level=0.95, title="", align=TRUE, single.row=TRUE, star.cutoffs=c(0.05,0.01,0.001), out="Table S4.html")
...
cumDist_descriptive<-c(0, cumsum(distr))

cumDist<-cbind(cumDist_normative, cumDist_descriptive)
...
### this is the model presented in Table S5
...
### output tables
stargazer(m2a, type="html", ci=TRUE, ci.level=0.95, title="", align=TRUE, single.row=TRUE, star.cutoffs=c(0.05/27,0.01/27,0.001/27), out="Table S5.html")
#stargazer(m2a, m2b, m2c, type="html", ci=TRUE, ci.level=0.95, title="", align=TRUE, single.row=TRUE, star.cutoffs=c(0.05/27,0.01/27,0.001/27), out="Table S5_robustness.html")
...
### these are the models presented in Table S6
...
stargazer(m2a, m2b, type="html" ,ci=TRUE, ci.level=0.95, title="", align=TRUE, single.row=TRUE,star.cutoffs=c(0.05,0.01,0.001), out="Table S6.html")
#stargazer(m2a, m2b, m2c,m2d, type="html" ,ci=TRUE, ci.level=0.95, title="", align=TRUE, single.row=TRUE,star.cutoffs=c(0.05,0.01,0.001), out="Table S6_robustness.html")
...
stargazer(m3a, m3b, type="html" ,ci=TRUE, ci.level=0.95, title="", align=TRUE, single.row=TRUE,star.cutoffs=c(0.05,0.01,0.001), out="Table S7.html")
#stargazer(m3c, m3d, type="html" ,ci=TRUE, ci.level=0.95, title="", align=TRUE, single.row=TRUE,star.cutoffs=c(0.05,0.01,0.001), out="Table S7_robustness.html")
...
stargazer(m1,m2, type="html" ,ci=TRUE, ci.level=0.95, title="", align=TRUE, single.row=TRUE,star.cutoffs=c(0.05/4,0.01/4,0.001/4), out="Table S8.html")
...
stargazer(m1,m2,m3, type="html" ,ci=TRUE, ci.level=0.95, title="", align=TRUE, single.row=TRUE,star.cutoffs=c(0.05,0.01,0.001), out="Table S9.html")
...
### linear models shown in Table S10
...
###### output table #########
stargazer(m1a,m1b, m2a,m2b, type="html" ,ci=TRUE, ci.level=0.95, title="", align=TRUE, single.row=TRUE,star.cutoffs=c(0.05,0.01,0.001), out="Table S10.html")
#stargazer(m1a,m1a_logistic, m1b, m1b_logistic, m2a, m2a_logistic, m2b, m2b_logistic, type="html" ,ci=TRUE, ci.level=0.95, title="", align=TRUE, single.row=TRUE,star.cutoffs=c(0.05,0.01,0.001), out="Table S10_robustness.html")

#########################

### behavioural task; Fig 4E ###

a <- read.xlsx("Why people follow rules - data.xlsx", sheet = 8, startRow = 1, colNames = TRUE)
...
###### output table #########
stargazer(m1, type="html" ,ci=TRUE, ci.level=0.95, title="", align=TRUE, single.row=TRUE,star.cutoffs=c(0.05,0.01,0.001), out="Table S11.html")
...
a<-read.table('simulation_results.txt', sep='\t', header=TRUE)
